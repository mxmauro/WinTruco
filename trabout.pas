...
uses Winprocs, Wintypes, OWindows, ODialogs, TruDlgs;
...
function FileSoundDialog(Path: PChar): Boolean;
...
const
  CreditText: array[1..5] of PChar = (
    '',
    'Truco para Windows V2.5',
    '',
    'Por Mauro H. Leggieri',
    '');
...
begin
  FileDialog :=Application^.ExecDialog(new(PFileDialog,
    Init(Application^.MainWindow, Which, Path))) = id_OK;
end;

function FileSoundDialog(Path: PChar): Boolean;
begin
  FileSoundDialog :=Application^.ExecDialog(new(PASoundDialog,
    Init(Application^.MainWindow, Path))) = id_OK;
end;
